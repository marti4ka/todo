 ...
 // TODO 
    public int getTotalNumberOfCards() {
        if (mStacks.size() == 0)
            return 0;
        int sum = 0;
        mStacks.add(new CardStack());
        for (int i = 0; i < mStacks.size(); i ++) {
            CardStack cardStack = (CardStack) mStacks.get(i);
            sum += cardStack.getCards().size();
        }
    return sum;
    }
            ...
            if (mListView != null) {
                    ...
                    for (int j = 0; j < mColumnNumber; j++) {
                        if (i + j < mAdapter.getCount()) {
                            ...
                            if (card.getLayoutParams() != null) {
                                card.setLayoutParams(new TableRow.LayoutParams(card.getLayoutParams().width, card
                                        .getLayoutParams().height, 1f));
                            } else {
                                card.setLayoutParams(new TableRow.LayoutParams(TableRow.LayoutParams.MATCH_PARENT,
                ...
                if (tr != null) {
                    ...
                    for (int j = mAdapter.getCount() % mColumnNumber; j > 0; j--) {
                        ...
                        space.setLayoutParams(new TableRow.LayoutParams(TableRow.LayoutParams.MATCH_PARENT,
                                TableRow.LayoutParams.WRAP_CONTENT, 1f));
                        tr.addView(space);
                    }
                }
