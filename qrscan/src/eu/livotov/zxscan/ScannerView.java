...
/**
 * (c) Livotov Labs Ltd. 2012 Date: 03/11/2014
 */
public class ScannerView extends FrameLayout implements CAMView.CAMViewListener {
	...
	protected void initUI() {
		final View root = LayoutInflater.from(getContext()).inflate(
				getScannerLayoutResource(), this);
	...
	/**
	 * Starts scanner, using particular camera. Use
	 * {@link #getAvailableCameras()} in order to get a list of all accessible
	 * cameras on this device
	 * 
	 * @param camInfo
	 */
	...
	public void setSameCodeRescanProtectionTime(
			long sameCodeRescanProtectionTime) {
	...
	public void setScannerViewEventListener(
			final ScannerViewEventListener scannerViewEventListener) {
	...
	public boolean onPreviewData(final byte[] bytes, final int i,
			final Camera.Size size) {
		final long currentTime = System.currentTimeMillis();

		if (decoderThread != null
				&& currentTime - lastDataSubmittedTimestamp > decodeThrottleMillis) {
			lastDataSubmittedTimestamp = currentTime;
			decoderThread.submitBarcodeRecognitionTask(bytes, size.width,
					size.height);
			return false;
		} else {
			return true;
	...
	private void processRecognizedBarcode(String data) {
		if (TextUtils.isEmpty(lastDataDecoded)
				|| !lastDataDecoded.equalsIgnoreCase(data)
				|| (System.currentTimeMillis() - lastDataDecodedTimestamp) > sameCodeRescanProtectionTime) {
